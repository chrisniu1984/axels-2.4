...
static void print_stat(axel_t *axel);
...
static void __human_size(char *sz, long size)
{
    #define KB  (1024)
    #define MB  (KB*KB)
    #define GB  (MB*KB)
    ...
    if (size > GB) {
        ...
        sprintf(sz, "%lu B", size);
        return;
    }

    sprintf(sz, "%.2f %s", s, unit);
}
...
void print_stat(axel_t *axel)
{
    char d[32];
    char s[32];
    char a[32];
    __human_size(d, axel->bytes_done);
    __human_size(a, axel->size);
    __human_size(s, axel->bytes_per_second);
    printf("\r[%llu%%] %s of %s -- %s/s ", axel->bytes_done*100/axel->size, d, a, s);
    fflush(stdout);
}
